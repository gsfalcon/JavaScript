...
var contador = Number(prompt("QUAL A CONTAGEM INICIAL?\nmax: 5000")) //Insira o valor desejado no contador
    ...
    while (contador > 0) {
        if (contador != 1) {
        document.write(contador + " " + palavra + "s" + " de cerveja no muro.<br>") //Texto no plural e no singular com if
        }
        if (contador != 1) {
    ...
    contador = contador - 1
        if (contador == 0) {
        document.write("Não restaram " + palavra + "s" + " de cerveja no muro...<br>") //Mensagem final, quando contador for ZERO
        }
    }